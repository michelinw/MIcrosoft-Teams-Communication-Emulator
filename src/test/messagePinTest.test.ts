import {
  ...
  requestDmSend, requestDmCreate, requestPin, requestJoin
} from './testHelper';
...
test('Testing pinning a valid message in a channel', () => {
  ...
  const pin = requestPin(user.output.token, message.output.messageId);
  expect(pin.output).toStrictEqual({});
  expect(pin.statusCode).toBe(OK);
});

test('Testing pinning a valid message in a DM', () => {
  const user = requestRegister('alan@unsw.edu.au', 'password123', 'Alan', 'Hattom') as AuthUserId;
...
test('Testing pinning an invalid message', () => {
  const user = requestRegister('alan@unsw.edu.au', 'password123', 'Alan', 'Hattom') as AuthUserId;
  const channel = requestCreate(user.output.token, 'My Channel', true) as ChannelId;
  const message = requestSend(user.output.token, channel.output.channelId, 'Hi, this is a message');
  const pin = requestPin(user.output.token, message.output.messageId + 1);
  expect(pin.output).toStrictEqual({ error: { message: expect.any(String) } });
  expect(pin.statusCode).toBe(INPUT_ERROR);
});

test('Testing double pin', () => {
  const user = requestRegister('matthew@unsw.edu.au', 'password123', 'Alan', 'Hattom') as AuthUserId;
  const channel = requestCreate(user.output.token, 'My Channel', true) as ChannelId;
  const message = requestSend(user.output.token, channel.output.channelId, 'Hi, this is a message');
...
test('Testing not channel owner', () => {
  ...
  const message = requestSend(user.output.token, channel.output.channelId, 'Hi, this is a message');
  const pin = requestPin(user2.output.token, message.output.messageId);
  expect(pin.output).toStrictEqual({ error: { message: expect.any(String) } });
...
test('Testing invalid token', () => {
  ...
  const channel = requestCreate(user.output.token, 'My Channel', true) as ChannelId;
  const message = requestSend(user.output.token, channel.output.channelId, 'Hi, this is a message');
  const pin = requestPin('', message.output.messageId);
  expect(pin.output).toStrictEqual({ error: { message: expect.any(String) } });
  expect(pin.statusCode).toBe(AUTH_ERROR);
});
